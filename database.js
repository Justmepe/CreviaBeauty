...
async function initializeDatabase() {
    ...
    try {
        ...
        if (SEED_DEMO) {
            ...
            if (parseInt(candleCount.rows[0].count) === 0) {
                await seedCandles(client);
            }
        }
...
async function seedCandles(client) {
    // Scented candles — the home-fragrance leg of the 5-component beauty box.
    // [name, description, price, original_price, discount, category, subcategory, image_url, stock, brand, scent_family]
    ...
    console.log('Candle products added to database');
}
